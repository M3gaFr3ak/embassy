...
//! injected conversion on the DFSDM filter, so `flt0.injected.read()` simply
//! awaits the next triggered result. The software `start_conversion()` calls
//! are gone.
//!
//! The filter produces ~400 Hz of conversions (2 MHz CKOUT / (FOSR=100 *
//! IOSR=50)); TIM1 is set slightly slower (350 Hz) so the filter always
//! finishes before the next trigger and never overruns.
//!
//! Pinout (same board as the PWM examples):
//! - A0   PA3     MIC_SEL
//! - A2   PC3_C   MIT_DAT
//! - A4   PC2_C   MIC_CLK
//! - PB14        LED PWM (TIM12)
...
async fn main(_spawner: Spawner) {
    ...
    //==================================================
    // Same as dfsdm_pwm_it_injected, but injected conversions are
    // triggered by TIM1 TRGO instead of software start_conversion().
    //==================================================
    ...
    // TIM1: 350 Hz trigger. Slightly slower than the filter's 400 Hz ceiling so
    // each conversion finishes before the next trigger (no overrun).
    let tim1 = Timer::new(p.TIM1);
    tim1.set_frequency(Hertz::hz(399), RoundTo::Slower);
